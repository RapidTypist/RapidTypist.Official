...
function initTyping() {
    ...
    if (charIndex < characters.length - 1 && timeLeft > 0) {
        ...
        // Scroll the typing text container to the bottom as the user types
        typingText.scrollTop = typingText.scrollHeight;

        let wpm = Math.round(((charIndex - mistakes) / 5) / (maxTime - timeLeft) * 420);
...
// Function to automatically scroll down to the latest content in the typing-text container
function autoScrollToBottom() {
    const typingText = document.querySelector('.typing-text');
    typingText.scrollTop = typingText.scrollHeight;
  }
  
  // Call this function when the content changes or is updated dynamically
  autoScrollToBottom();
